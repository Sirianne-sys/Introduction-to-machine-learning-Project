...
#confusion matrix
rf_conf_matrix<-confusionMatrix(rf_prediction, test$Species, mode = "everything", positive="1") 
#rf_conf_matrix
```
...
```{r}
set.seed(123)
cv_svm_results <- train(Species ~ ., data = train, method = "svmLinear", trControl = trainControl(method = "cv", number = 10))
cv_svm_results
```
